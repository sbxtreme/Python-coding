...
def create_connection_connectdb():
	...
	try:
		...
		con = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};\
			SERVER='+'lt-app-db.stg.local'+';DATABASE='+'finnair'+';UID='+'loyaltree'+';PWD='+'B4h5lhsl3_Kqit8AF3RoIxuLeMs4aRqQ')
...
def create_connection_coredb():
	...
	try:
		...
		# hardcoding
		con=pymysql.connect(host="core-db-finnair.stg.local",user="sessionm",password="lGZ7OqSMts0pziKb",db="greyhound_integration_finnair")
...
def tier_validation(tier_file,core_con,connect_con):
	...
	try:
		...
		df_tier_file=pd.read_csv(tier_file,skipinitialspace=True, usecols=['external_id','tier_name'])
		...
		sql_on_connect="""select 
							...
							from incent.TierMembers tm 
							inner join incent.TierSystems ts on tm.TierSystemID=ts.id
							inner join incent.TierLevels tl on tl.TierSystemID= ts.id and tl.id=tm.tierlevelid
							where ts.id= (select id from incent.TierSystems where name='Finnair NextGen')
							and tm.RetailerID = (select id from retailers where name = 'finnair')
		...
		#print(df_userid_tiername_connect)

		#print("Records from coredb:",df_userid_extid_core.shape[0])
		#print("Records from connectdb:",df_userid_tiername_connect.shape[0])
		...
		'''
		if not df_external_id_not_found.drop_duplicates().empty:
			logger.info(df_external_id_not_found.drop_duplicates().to_string(index=False))	

		tier_preference_list=['PlatinumLumo','Platinum','Gold','Silver','Basic']

		# logic to get max tier based on the preference list and then merge with the master dataframe for comparision

		distinct_external_id=df_tier_file.external_id.unique()

		# final dataframe created to hold highest tier of each user based on external_id and tier_name
		df_tier_file_final=pd.DataFrame(columns=['external_id','tier_name'])

		for external_id in distinct_external_id:
			if not (df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[0])]).empty:
				df_tier_file_final=df_tier_file_final.append(df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[0])])

			elif not (df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[1])]).empty:
				df_tier_file_final=df_tier_file_final.append(df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[1])])

			elif not (df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[2])]).empty:
				df_tier_file_final=df_tier_file_final.append(df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[2])])

			elif not (df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[3])]).empty:
				df_tier_file_final=df_tier_file_final.append(df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[3])])

			else:
				df_tier_file_final=df_tier_file_final.append(df_tier_file[(df_tier_file['external_id'] == external_id) & (df_tier_file['tier_name'] == tier_preference_list[4])])


		# Get unique highest tier values for each external id
		df_tier_file_final=df_tier_file_final.drop_duplicates()
		'''

		# to get records whose file tier does not match with database tier
		#df_mismatch=df_tier_file_final.merge(master_df, on='external_id').query('tier_name_x != tier_name_y')
...
def main(argv=None):
	...
	try:
		...
		# the below commented and some args changed for testing
